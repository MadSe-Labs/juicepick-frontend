...
import { Star, Bell, Share2, Heart, ShoppingCart, ArrowLeft } from "lucide-react"
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: "나스티 쿠션맨 액상의 상세 정보, 최저가 비교, 리뷰를 확인하세요.",
  }
}
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="container mx-auto px-4 py-8">
        {/* 뒤로가기 */}
        <div className="mb-4">
          <Link href="/" className="inline-flex items-center text-gray-600 hover:text-rose-500 transition-colors">
        ...
        <div className="flex flex-col lg:flex-row gap-6">
                ...
                <div className="w-full md:w-3/5">
                  ...
                  <div className="mb-4">
                    <div className="flex items-baseline">
                      <span className="text-2xl font-bold text-green-600">{formatPrice(product.lowestPrice)}원</span>
                      <span className="text-sm text-gray-500 line-through ml-2">
                        {formatPrice(product.averagePrice)}원
                      </span>
                      <span className="ml-2 text-sm font-medium text-green-600">{product.discountRate}% 할인</span>
                    </div>
                    <div className="text-xs text-gray-500 mt-1">{product.sellers.length}개 판매처</div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div className="bg-gray-100 px-3 py-2 rounded flex justify-between">
                      ...
                      <span className="font-medium">{product.specs.nicotine}</span>
                    </div>
                    <div className="bg-gray-100 px-3 py-2 rounded flex justify-between">
                    ...
                    <div className="bg-gray-100 px-3 py-2 rounded flex justify-between">
                      ...
                      <span className="font-medium">{product.specs.flavor}</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="flex flex-col md:flex-row gap-6 mb-6 p-4 bg-gray-50 rounded-lg">
                <div className="text-center md:w-1/4">
                  <div className="text-5xl font-bold text-rose-500 mb-2">{product.rating}</div>
                  <div className="flex items-center justify-center text-yellow-400 mb-1">
                    {[...Array(5)].map((_, i) => (
                      <Star key={i} className={`h-5 w-5 ${i < Math.floor(product.rating) ? "fill-current" : ""}`} />
                    ))}
                  </div>
                  <div className="text-gray-500">{product.reviewCount}개 리뷰</div>
                </div>
                ...
                <div className="md:w-3/4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {product.reviewTags.map((tag, index) => (
                      <div key={index} className="px-3 py-1 bg-white border border-gray-200 rounded-full text-sm">
          ...
          <div className="w-full lg:w-64 space-y-6">
            ...
            <div className="bg-white p-4 rounded-lg shadow">
              ...
              <PriceTrendChart />
              <p className="text-xs text-gray-500 mt-2">최근 30일 가격 변동</p>
            </div>

            <div className="bg-white p-4 rounded-lg shadow">
              <h3 className="font-bold text-lg mb-3">최저가 알림</h3>
              <p className="text-sm text-gray-600 mb-3">원하는 가격에 도달하면 알려드립니다</p>
              <div className="flex mb-3">
                ...
                />
                <span className="px-3 py-2 bg-gray-100 border-t border-r border-b rounded-r-lg">원</span>
              </div>
            ...
            <div className="bg-white p-4 rounded-lg shadow">
              ...
              <div className="space-y-3">
                {[1, 2, 3].map((item) => (
                  <Link href={`/product/${item}`} key={item} className="flex gap-2 group">
                    ...
                    <div>
                      ...
                      <div className="text-green-600 text-sm font-medium">15,000원</div>
